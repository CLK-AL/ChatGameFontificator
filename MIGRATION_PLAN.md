...
| R5 | `2589476` | `ConfigMessage` + `MessageCasing` + `UsernameCaseResolutionType` | Immutable `data class` + `validate()` + round-trip; reuses `baseValidation` with `spaceAllowedKeys = setOf(KEY_MESSAGE_CONTENT_BREAK)`. `timeFormatter` elided (JVM-side concern); raw `timeFormat` pattern preserved. §14 garbled Javadoc replaced. | 120 | 6 field-exact |
| R6 | `23916e4` | `ConfigChat` + `ConfigColor` + `ConfigCensor` — **completes config layer port** | `ConfigChat` (20 fields incl. chroma border decomposed from `Rectangle` to 4 ints). `ConfigColor` (5 named colors + palette + booleans; introduced `ColorRGBA` data class replacing `java.awt.Color` with `fromHex/toHex/toArgbInt` round-trip). `ConfigCensor` (6 required + 3 optional comma-separated list fields). | 186 | 12 field-exact across all three configs |
